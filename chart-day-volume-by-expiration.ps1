...
# ----------------------------------------------------------------------
# chart volume for all contracts by expiration
# ----------------------------------------------------------------------
...
# generate call datasets 

$calls_by_expiration = $all_candles | ? contract_type -EQ call | Group-Object expiration_date

# $expiration_group = $calls_by_expiration[0]

$i = 0

$datasets_calls = foreach ($expiration_group in $calls_by_expiration)
{
    $groups = $expiration_group.Group | Group-Object date | Sort-Object Name
    ...
    @{
        label = ('C {0}' -f $expiration_group.Name)
        data = $call_volume
        backgroundColor = $colors[$i++ % $colors.Count]
    }
}

# generate put datasets 

$puts_by_expiration = $all_candles | ? contract_type -EQ put | Group-Object expiration_date

$i = 0

# $datasets_puts = foreach ($expiration_group in $puts_by_expiration)
# {
#     $groups = $expiration_group.Group | Group-Object date | Sort-Object Name
...
#     @{
#         label = ('P {0}' -f $expiration_group.Name)
#         data = $put_volume
#         backgroundColor = $colors[$i++ % $colors.Count]
#     }
# }
...
$datasets_puts = foreach ($expiration_group in $puts_by_expiration)
{
    ...
    $put_volume = $labels | ForEach-Object { 

        $result = $volume_table | ? Name -EQ $_

        if ($result -eq $null) { 0 } else { $result | % volume }
    }

    @{
        label = ('P {0}' -f $expiration_group.Name)
        data = $put_volume | ForEach-Object { $_ }
        backgroundColor = $colors[$i++ % $colors.Count]
    }
}
        ...
        options = @{
            title = @{ 
                ...
                text = @(
                    ...
                    # 'volume'
                    ('{0} volume' -f $contracts[0].underlying_ticker)
                    ) 
            }
...
exit
# ----------------------------------------------------------------------
.\chart-day-volume-by-expiration.ps1 -symbol IWM -date 2023-07-14

.\chart-day-volume-by-expiration.ps1 -symbol DM -date 2023-07-14
